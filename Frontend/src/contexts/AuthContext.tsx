...
const API_URL = "https://cancerprognosis.onrender.com/api";
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const resendVerificationCode = async (email: string): Promise<string> => {
    setIsLoading(true);
    try {
      const res = await axios.post(`${API_URL}/auth/resend-verification-code`, { email });
      return res.data.message;
    } catch (err: any) {
      throw new Error(err.response?.data?.message ?? "Resend failed");
    } finally {
      setIsLoading(false);
    }
  };
